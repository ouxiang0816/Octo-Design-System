...
CHANGELOG_JSON = WORKFLOW_GENERATED_DIR / "ai-reference-changelog.json"
...
IMPLEMENTED_COMPONENTS = {
    ...
    "component.split-tabs": ("SplitTabs", COMPONENTS_SOURCE, DEMOS_SOURCE),
    "component.selecter-tabs": ("SelecterTabs", COMPONENTS_SOURCE, DEMOS_SOURCE),
    "component.progress": ("Progress", COMPONENTS_SOURCE, DEMOS_SOURCE),
}
...
def _figma_fallback_spec(entry: Entry) -> str:
    figma = entry.figma or {}
    extracted = figma.get("extracted", {}) if isinstance(figma.get("extracted"), dict) else {}
    summary = figma.get("summary", "")
    parts: list[str] = []
    if summary:
        parts += [f"> {summary}", ""]
    if extracted:
        parts += ["| 属性 | 值 |", "|---|---|"]
        for key, label in [("size", "尺寸"), ("colors", "颜色"), ("layout", "布局"), ("variants", "变体/状态")]:
            if extracted.get(key):
                parts.append(f"| {label} | {extracted[key]} |")
        parts.append("")
    parts.append("_以上数据来自 Figma 自动采集，人工规范待补充。_")
    return "\n".join(parts)


def _update_ai_reference_changelog(implemented: list[Entry]) -> list[dict]:
    current_ids = sorted(e.id for e in implemented)
    existing: list[dict] = []
    if CHANGELOG_JSON.exists():
        try:
            existing = json.loads(CHANGELOG_JSON.read_text(encoding="utf-8"))
        except (json.JSONDecodeError, Exception):
            existing = []
    prev_ids = set(existing[-1].get("componentIds", [])) if existing else set()
    id_to_label = {e.id: (e.cn_name or e.name) for e in implemented}
    added = [cid for cid in current_ids if cid not in prev_ids]
    removed = [cid for cid in prev_ids if cid not in set(current_ids)]
    record: dict = {
        "timestamp": datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC"),
        "total": len(current_ids),
        "componentIds": current_ids,
        "added": added,
        "removed": removed,
        "addedLabels": [id_to_label.get(cid, cid) for cid in added],
    }
    existing.append(record)
    existing = existing[-10:]
    WORKFLOW_GENERATED_DIR.mkdir(parents=True, exist_ok=True)
    CHANGELOG_JSON.write_text(json.dumps(existing, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return existing
...
def generate_ai_reference(entries: list[Entry]) -> None:
    ...
    changelog = _update_ai_reference_changelog(implemented)
    ...
    id_to_label = {e.id: (e.cn_name or e.name) for e in implemented}
    for record in reversed(changelog[-5:]):
        ts = record["timestamp"]
        total = record["total"]
        added_labels = record.get("addedLabels") or [id_to_label.get(cid, cid) for cid in record.get("added", [])]
        if added_labels:
            lines.append(f"- `{ts}` — 共 {total} 个组件，新增 {len(added_labels)} 个：{'、'.join(added_labels)}")
        else:
            lines.append(f"- `{ts}` — 共 {total} 个组件，无变更")
    lines += ["", "---", "", "## 使用方式",
        "",
    ...
    for entry in implemented:
        ...
        clean_spec = strip_figma_auto_blocks(entry.md.text) if (entry.md and entry.md.text) else ""
        if clean_spec:
            lines.append(clean_spec)
        elif entry.figma and (entry.figma.get("summary") or entry.figma.get("extracted")):
            lines.append(_figma_fallback_spec(entry))
        else:
