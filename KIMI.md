# Octo Design System — Kimi Code 使用指南

当用户要求"用 Octo 设计系统"、"按设计规范"或"用设计系统组件"生成 UI 代码时，遵循以下流程。

## 第一步：读取规范

从以下 URL 获取最新设计规范（每次任务开始时必须重新获取）：
...
Octo Design System **没有 npm 包**。对于规范中已实现的组件（`AI_REFERENCE.md` 可用组件表中 `exportName` 列有值），必须从 GitHub 获取源码并复制到目标项目，**禁止自己重新实现**。
